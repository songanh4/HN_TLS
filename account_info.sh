...
ACCOUNT_INFO_HDFS_PATH_FLAG=/DATAWH/FLAG/CUSTOMER_360_FLAG/${DAY_KEY}
HDFS_PATH=/DATAWH/TMP/SOLR/ACCOUNT_INFO/*.csv
WORK_DIR_DATA=/solr/solr_script/account_info/data
WORK_DIR=/solr/solr_script/account_info/
solr_endpoint=http://10.3.21.75:8985/solr/account_info/update

LOG_LOCATION=/solr/solr_script/account_info/logs/
...
echo 'ACCOUNT_EXTEND_INFO_HDFS_PATH:'$ACCOUNT_INFO_HDFS_PATH_FLAG
...
do
  ...
  hdfs dfs -test -d $ACCOUNT_INFO_HDFS_PATH_FLAG    
  if [ ${?} = 0 ] 
  then     
    ...
    spark-submit --driver-memory 20g --num-executors 10 --executor-cores 5 --executor-memory 5g  ${WORK_DIR}account_info.py app_name=ACCOUNT_INFO_EXP workflow_name=ACCOUNT_INFO_EXP day_key=$DAY_KEY
    ...
    #find $WORK_DIR_DATA/*.csv -type f | xargs -P 2 -I {} curl -i -k --negotiate -u: http://10.3.21.75:8985/solr/account_info_c/update -H "Content-Type: application/csv" --data-binary @{} \;    
    ...
    echo "Data has not been aggregated ACCOUNT_INFO day_key:"$DAY_KEY 
  fi 
done